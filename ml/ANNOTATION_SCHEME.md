...
| 脚本 | 作用 | 运行时机 |
|------|------|---------|
| `ml/train/gen_5class_xmls.py` | 从 `Annotations/` 生成 5 类副本到 `Annotations_5class/` | 每次新增/修改标注后 |
| `ml/classifier/prepare_data.py` | 从 `Annotations/` 的 7 类标注裁剪 L/H ROI 到 `classifier/data/` | 新增标注或需要重新生成分类器数据集时 |
| `ml/train/gen_fp_txt.py` | 生成 train/val/test 文件列表 | 新增图片后 |
