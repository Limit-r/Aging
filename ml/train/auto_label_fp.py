...
用法:
  python ml/train/auto_label_fp.py
  python ml/train/auto_label_fp.py --conf 0.15        # 模糊帧可调低阈值
  python ml/train/auto_label_fp.py --range 000470 000535   # 只处理指定帧号区间
  python ml/train/auto_label_fp.py --image frame_000536     # 只处理单帧
  python ml/train/auto_label_fp.py --no-vis           # 不生成预览图
  python ml/train/auto_label_fp.py --overwrite        # 覆盖已存在的同名单帧 XML
"""
import argparse
import os
import sys
...
# 模型/训练代码根 = ml/  (auto_label_fp.py -> train -> ml)
