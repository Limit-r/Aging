...
设计说明
--------
本脚本不再自己实现训练流程, 而是直接复用根目录 `train.py` 的 `train(cfg)` 函数 ——
即"参考的成体系 yolo 训练脚本"。这样:
  - 模型/损失/数据加载/EMA/评估/早停/断点续训 等所有成熟组件零重复实现
  - 配置完全由 config_fp_train_v3.json 驱动, 字段与根目录 train.py 期望一致
  - 仅需保证 PROJECT_ROOT 加入 sys.path, 让 `from train import train` 可达
...
用法
----
  python led_pipeline/train/train_fp.py
  python led_pipeline/train/train_fp.py --config path/to/other.json
"""
import json
import os
import sys

# PROJECT_ROOT = d:\YOLO_train  (本文件向上 3 层: train_fp.py -> train -> led_pipeline -> root)
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if PROJECT_ROOT not in sys.path:
    sys.path.insert(0, PROJECT_ROOT)

# 复用根目录成体系训练脚本 (train.py 的 train(cfg) 函数)
# 注意: 不能用 `from train import train` —— led_pipeline/train/ 包会遮蔽
# led_pipeline/train.py 模块。改用 importlib 按文件路径显式加载。
