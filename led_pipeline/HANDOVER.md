...
| 配置文件 | [config_fp_train_v3.json](file:///d:/YOLO_train/led_pipeline/train/config_fp_train_v3.json) |
...
|---|---|
| 模型 | TinyConv（6.7K 参数） |
| 权重位置 | `led_pipeline/classifier/weights/best.pth` |
| 训练数据 | 1584 L（灭） + 258 H（亮），6.1:1 比例 |
| 测试准确率 | 99.5% |
| 推理接口 | [classifier/infer.py](file:///d:/YOLO_train/led_pipeline/classifier/infer.py) → `LEDClassifier.predict(roi_bgr)` |
...
4. 消融实验代码位于 `led_pipeline/ablation/`，可随时复现
...
| 文件 | 变更说明 |
|---|---|
| [detect_fp_video.py](file:///d:/YOLO_train/led_pipeline/detect/detect_fp_video.py) | 集成 5 类 YOLO + TinyConv 分类器，新增 `classify_led_dets()` 函数；权重路径更新为 v2 |
| [infer_fp.py](file:///d:/YOLO_train/led_pipeline/detect/infer_fp.py) | 默认权重路径更新为 `FP_v3_5classes_v2` |
| [gen_fp_txt.py](file:///d:/YOLO_train/led_pipeline/train/gen_fp_txt.py) | 移除 CLASS_MAP 映射，直接解析 XML 中的 5 类名称 |
| [config_fp_train_v3.json](file:///d:/YOLO_train/led_pipeline/train/config_fp_train_v3.json) | save_dir 改为 `FP_v3_5classes_v2`，类别数 5 类 |
...
| 文件 | 说明 |
|---|---|
| [classifier/model.py](file:///d:/YOLO_train/led_pipeline/classifier/model.py) | TinyConv 模型定义（6.7K 参数） |
| [classifier/train.py](file:///d:/YOLO_train/led_pipeline/classifier/train.py) | 二分类器训练脚本 |
| [classifier/infer.py](file:///d:/YOLO_train/led_pipeline/classifier/infer.py) | 二分类器推理接口 |
| [classifier/prepare_data.py](file:///d:/YOLO_train/led_pipeline/classifier/prepare_data.py) | 从标注裁剪 ROI 生成 L/H 数据集 |
| [train/update_xml_to_5classes.py](file:///d:/YOLO_train/led_pipeline/train/update_xml_to_5classes.py) | 批量将 XML 从 7 类更新为 5 类 |
...
**文件位置**：`led_pipeline/esp32cam/esp32cam_recorder/esp32cam_recorder.ino`（已从 `esp32_cam_recorder/` 迁移）
...
│   ├── train_fp.py                  # 训练入口（调用根目录 train.train）
│   └── update_xml_to_5classes.py    # 批量将 XML 从 7 类更新为 5 类
│
├── detect/                          # 推理检测相关
│   ├── infer_fp.py                  # 验证集/测试集批量推理评估（默认512×512，v2权重）
│   ├── detect_fp_video.py           # 单路视频检测（5类YOLO + TinyConv二分类器）
│   ├── fp_cascade_pipeline.py       # 三级递进检测 pipeline（历史保留）
│   └── outputs/                     # 推理输出（vis_*.jpg, det_*.mp4）
│       ├── FP_v2/val/               # val 集可视化（旧7类）
│       ├── FP_v2/test/              # test 集可视化（旧7类）
│       ├── FP_v3_5classes/          # 旧5类推理结果（277张）
│       ├── FP_v3_5classes_v2/       # 新5类推理结果（337张，当前最新）
│       └── FP_v2_new/               # 视频检测结果
│
...
│       └── best.pth                 # 训练好的二分类权重
│
├── esp32cam/                        # ESP32-CAM 边缘部署
│   ├── esp32cam_stream/             # 固件 A：视频流采集（用于 PC 端 YOLO 推理）
│   │   └── esp32cam_stream.ino      #   WiFi连接 + MJPEG 流服务器（OV2640）
│   ├── esp32cam_recorder/           # 固件 B：手机端录制
│   │   └── esp32cam_recorder.ino    #   Canvas+MediaRecorder 录制，无需 SD 卡
│   ├── pc_yolo_detect.py            # PC 端单路拉流+YOLO推理（需扩展为多路）
│   ├── outputs/                     # 推理结果保存目录
│   └── README.md                    # ESP32-CAM 部署说明
│
├── ablation/                        # 消融实验（可随时复现）
│   ├── configs/                     # 12 组实验配置文件
...
│
└── archive/
    └── infer_fp.py                  # 旧批量验证脚本（归档）
```

### 通用框架（根目录，非 led_pipeline 专属）

- `train.py` —— 通用训练入口
- `config.py` —— 数据集配置（含 FP 的 label_list，`input_shape` 已改为 `[512,512]`）
...
```powershell
# 重建三分 txt（新增标注后必须执行）
python led_pipeline\train\gen_fp_txt.py
```
...
python led_pipeline\train\train_fp.py

# 二分类器训练
python led_pipeline\classifier\train.py
```
...
python led_pipeline\detect\infer_fp.py --split val --conf 0.25

# test 集评估
python led_pipeline\detect\infer_fp.py --split test --conf 0.25

# 单路视频检测（512×512，5类YOLO + TinyConv二分类器）
python led_pipeline\detect\detect_fp_video.py --video FP03.mp4 --conf 0.20
```

### ESP32-CAM 操作

```powershell
# 方案 A：PC 端拉流 YOLO 推理（烧录 esp32cam_stream.ino）
python led_pipeline\esp32cam\pc_yolo_detect.py --url http://<ESP_IP>/stream

# 方案 B：手机端录制视频采集数据（烧录 esp32cam_recorder.ino）
...
python led_pipeline\train\update_xml_to_5classes.py
```
...
```powershell
# 一键运行所有 12 组实验
python led_pipeline\ablation\scripts\run_ablation.py
```
