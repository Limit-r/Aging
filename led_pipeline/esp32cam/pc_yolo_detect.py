...
用法:
  # 1. 先烧录 esp32cam_stream.ino 到 ESP32-CAM, 串口会打印 IP (例如 192.168.43.123)
  # 2. PC 连同一手机热点 (QH), 运行:
  python led_pipeline/esp32cam/pc_yolo_detect.py --url http://192.168.43.123/stream

  # 指定权重/阈值
  python led_pipeline/esp32cam/pc_yolo_detect.py --url http://192.168.43.123/stream \
      --weights weights/FP_v2/best_epoch_weights.pth --conf 0.25 --nms 0.45
"""
import argparse
import os
import sys
...
# PROJECT_ROOT = d:\YOLO_train  (本文件向上 2 层: pc_yolo_detect.py -> esp32cam -> led_pipeline -> root)
...
DEFAULT_WEIGHTS = str(PROJECT_ROOT / 'weights' / 'FP_v2' / 'best_epoch_weights.pth')
DEFAULT_LABELS  = str(PROJECT_ROOT / 'datasets' / 'FP' / 'label.txt')
DEFAULT_OUTDIR  = str(PROJECT_ROOT / 'esp32cam' / 'outputs')
