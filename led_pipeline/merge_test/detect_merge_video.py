# -*- coding: utf-8 -*-
"""
FP + A 合并视频检测脚本 (9 类 YOLO + TinyConv 二分类)
...
用法:
  python led_pipeline/merge_test/detect_merge_video.py --video D:\Aging\Video\001.mp4
  python led_pipeline/merge_test/detect_merge_video.py --video D:\Aging\Video\001.mp4 --conf 0.25
"""
import argparse
import os
import sys
...
sys.path.insert(0, str(MERGE_ROOT.parent))
from classifier.model import TinyConv

# ===== 默认路径 (合并模型) =====
MERGE_WEIGHTS  = str(MERGE_ROOT / 'weights' / 'model_best_precision_deploy.pt')
MERGE_LABELS   = str(MERGE_ROOT / 'label_merge.txt')
MERGE_CLF_WEIGHTS = str(MERGE_ROOT / 'weights' / 'best_tinyconv_merge.pth')
MERGE_OUTDIR   = str(MERGE_ROOT / 'outputs')

# 显示颜色 (BGR) — 按合并后的 9 类基础类别
BASE_COLORS = {
    'FP_SIG_area': (0, 200, 0),     # 绿
    'FP_PWR_area': (0, 255, 0),     # 亮绿
    'FP_VPL':      (255, 100, 100), # 浅红
    'FP_CPL':      (255, 0, 0),     # 蓝
    'FP_PWR':      (0, 255, 255),   # 黄
...
# 定义不需要做 H/L 分类的 area 类别名
AREA_CLASS_NAMES = {'FP_SIG_area', 'FP_PWR_area', 'A_area'}
...
class FlashTracker:
    ...
    def update(self, frame_idx, dets, class_names, hl_labels, frame_w):
        ...
        for base_cn, items in led_groups.items():
            items.sort(key=lambda x: (x[0][0] + x[0][2]) / 2.0)
    ...
    def print_summary(self):
        print('')
        print('=' * 60)
        print('LED Flash Statistics (FP + A Merge)')
        print('=' * 60)
        for lid in self.led_ids:
            if self.last_state[lid] is not None:
                final_state = 'High' if self.last_state[lid] == 1 else 'Low'
                print('  %-15s  Flashes: %-3d  Final: %s' % (lid, self.flash_counts[lid], final_state))
        print('=' * 60)


def main():
    parser = argparse.ArgumentParser(description='FP+A 合并视频检测 (9 类 YOLO + TinyConv)')
    parser.add_argument('--video', required=True, help='输入视频路径')
    parser.add_argument('--weights', default=MERGE_WEIGHTS)
    parser.add_argument('--labels', default=MERGE_LABELS)
    parser.add_argument('--clf-weights', default=MERGE_CLF_WEIGHTS)
    parser.add_argument('--outdir', default=MERGE_OUTDIR)
    parser.add_argument('--phi', default='n')
    parser.add_argument('--conf', type=float, default=0.25)
    parser.add_argument('--nms', type=float, default=0.45)
    parser.add_argument('--no-preview', action='store_true',
                        help='非预览模式: 全力加速处理, 不显示画面, 快速得到结果')
    ...
    out_path = outdir / ('det_merge_' + Path(args.video).stem + '.mp4')
    ...
    mode_label = '预览模式 (正常速度)' if preview else '非预览模式 (全力加速)'
    print('=' * 60)
    print('开始检测 [%s]' % mode_label)
    if preview:
        print('操作: q=退出, 空格=暂停')
    print('=' * 60)
    ...
    while True:
        if not paused:
            ...
            for i, d in enumerate(dets):
                ...
                cn = '%s_%s' % (base_cn, hl) if hl and base_cn not in AREA_CLASS_NAMES else base_cn
                det_summary[cn] = det_summary.get(cn, 0) + 1
            print('  frame %4d/%d  fps %.1f  %s' % (frame_idx, total, cur_fps, det_summary), end='\r')

        if preview:
            # 预览模式: 按原始视频速度播放
            cv2.imshow('Merge detect (q=quit, space=pause)', frame)
            if not paused:
                # 计算剩余等待时间, 使帧间隔匹配原始视频帧率
                now = time.time()
                if frame_idx == 1:
                ...
                if key == ord('q'):
                    ...
                    # 如果落后太多（超过 2 帧）, 直接跳到当前时间
                    if next_frame_time < time.time() - frame_interval * 2:
