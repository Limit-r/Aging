# -*- coding: utf-8 -*-
"""
合并 FP + A 系列 LED 亮灭二分类模型 (TinyConv)。

用法:
    python led_pipeline/merge_test/train_clf_merge.py

输出:
    merge_test/weights/best_tinyconv_merge.pth
"""
import argparse
import os
import sys
...
DATA_DIR = PROJECT_ROOT / 'merge_test' / 'clf_data'
OUTPUT_DIR = PROJECT_ROOT / 'merge_test' / 'weights'
INPUT_SIZE = 32
...
def main():
    ...
    save_path = OUTPUT_DIR / f'best_{args.model}_merge.pth'
    ...
    print()
    print('=' * 60)
    print(f'合并分类器测试集: loss={test_loss:.4f}  acc={test_acc:.4f}')
    print('=' * 60)

    try:
        model.eval()
        dummy = torch.randn(1, 3, INPUT_SIZE, INPUT_SIZE, device=device)
        onnx_path = str(OUTPUT_DIR / f'best_{args.model}_merge.onnx')
