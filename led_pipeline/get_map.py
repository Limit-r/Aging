...
if __name__ == "__main__":
    ...
    #--------------------------------------------------------------------------------------#
    #   MINOVERLAP 表示“最小重叠度”，也就是IoU（Intersection over Union，交并比）阈值
    ...
    image_ids = [os.path.splitext(os.path.basename(l.split()[0]))[0]
                 for l in open(os.path.join(VOCdevkit_path, "FP/2025_test.txt")).read().strip().split('\n')
                 if l.strip()]
    ...
    if map_mode == 0 or map_mode == 1:
        ...
        for image_id in tqdm(image_ids):
            image_path  = os.path.join(VOCdevkit_path, "FP/JPEGImages/"+image_id+".jpg")
    ...
    if map_mode == 0 or map_mode == 2:
        ...
        for image_id in tqdm(image_ids):
            with open(os.path.join(map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
                root = ET.parse(os.path.join(VOCdevkit_path, "FP/Annotations/"+image_id+".xml")).getroot()
